...
describe('bridged-functions', () => {
  type Bus = {
    fn1: (num: number, str: string) => string | Promise<string>;
    fn2: () => Promise<void>;
  };

  let bridge: ReturnType<typeof createBridge<Bus>>;

  beforeEach(() => {
    bridge = createBridge<Bus>({
  ...
  test('calls latest registered function', async () => {
    const cb = jest.fn().mockImplementation((num: number, str: string) => `${str}-${num}`);
  ...
  test('deregister falls back to previous function', async () => {
    const cb1 = jest.fn().mockImplementation((num: number, str: string) => `cb1:${str}-${num}`);
    const cb2 = jest.fn().mockImplementation((num: number, str: string) => `cb2:${str}-${num}`);
  ...
  test('deregister works even if wrong order', async () => {
    const cb1 = jest.fn().mockImplementation((n: number, s: string) => `cb1:${s}-${n}`);
    const cb2 = jest.fn().mockImplementation((n: number, s: string) => `cb2:${s}-${n}`);
    const cb3 = jest.fn().mockImplementation((n: number, s: string) => `cb3:${s}-${n}`);
  ...
  test('using returned deregister function works', async () => {
    const cb = jest.fn().mockImplementation((n: number, s: string) => `cb:${s}-${n}`);
